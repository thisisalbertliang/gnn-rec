...
if __name__ == "__main__":
    ...
    for algo in algos:
        ...
        # print(' & '.join("{:.3f}".format(rmse) for rmse in out["test_rmse"]) + f" & {mean_rmse}")
        # print(' & '.join("{:.3f}".format(mae) for mae in out["test_mae"]) + f" & {mean_mae}")
        
        new_line = [link]
        new_line.extend("{:.3f}".format(rmse) for rmse in out["test_rmse"])
        new_line.extend("{:.3f}".format(mae) for mae in out["test_mae"])
        new_line.extend((mean_rmse, mean_mae, cv_time))

        print(tabulate([new_line], tablefmt="pipe"))  # print current algo perf
        table.append(new_line)

    header = [LINK[dataset]]
    header.extend(f"Fold {i} RMSE" for i in range(1, 6))
    header.extend(f"Fold {i} MAE" for i in range(1, 6))
    header.extend(("RMSE", "MAE", "Time"))
