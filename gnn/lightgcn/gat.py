...
class GAT(object):
    ...
    def __init__(self, hparams, data, seed=None):
        ...
        self.gat = GraphAttentionLayer(self.emb_dim, self.emb_dim, dropout=0.5, alpha=0.2)
    ...
    def _create_lightgcn_embed(self, adj_to_dense=True):
        ...
        if adj_to_dense:
            A_hat = tf.sparse.to_dense(A_hat)
        ...
        for k in range(0, self.n_layers):
            ...
            if adj_to_dense:
                # ego_embeddings = tf.matmul(A_hat, ego_embeddings) # (2625, 64)
                ego_embeddings = self.gat(ego_embeddings, A_hat)
            else:
